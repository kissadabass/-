...
namespace The_projact_603410194_3
{
    public partial class Form1 : Form
    {
        ...
        {
            //Button 2,5
            CheckButton(button1, button2); //การทดสอบ
        ...
        {
            count = count + 1; //จำนวนเพิ่ม 1 จนถึง 15
            label2.Text = count + " ครั้ง";
            if (button1.Text == "1" && 
                button2.Text == "2" && 
                button3.Text == "3" && 
                button4.Text == "4" && 
                ...
                button16.Text == "")
            {
                ...
                MessageBox.Show("คุณชนะ " + count + " จำนวนครั้งที่กด"); //จะเเสดงว่าชนะ เเละ จำนวนที่กด
        ...
        public void bass()
        {
            ...
            int i, j, RN; // ตั้งตัวแปร
            int[] a = new int[16]; //เเรนดอมทั้ง 16
            Boolean flag = false; //ให้flag เท็จ ตรวจหาเท็จ
            i = 1;
            //a[j] = 1;
            do
            {
                ...
                //int b = rnd.Next(1, 15);
                RN = Convert.ToInt32((rnd.Next(0, 15)) + 1);//จะเเรนดอม 1- 15
                for (j = 1; j <= i; j++)
                {
                    if (a[j] == RN) //ถ้า a 
                    {
            ...
            while (i <= 15); //ข้อเเม้ 15 มากกว่า = i
            button1.Text = Convert.ToString(a[1]); // แปลงค่าเป็น string
        ...
        {
            bass(); //เมตตอท มาใช้ใน from เพื่อเเรนดอม
        }

        private void คำแนะนำToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MessageBox.Show("ตั้งสติ ก่อนสตาร์ท");
        }
